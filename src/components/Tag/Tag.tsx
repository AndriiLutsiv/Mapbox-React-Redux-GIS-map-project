...
const Tag: React.FC<Props> = ({ id, areaName, scenarioName, projectName, onEdit, onDelete }) => {
    return <>
        <Tooltip
            title={`${areaName}, ${scenarioName}, ${projectName}`}
            overlayStyle={{ maxWidth: '600px' }}
            color={'#292929'}>
            <div className={styles.tag} style={{ color: 'white' }}>
                <p><span>{areaName}</span>,</p>
                <p><span>{scenarioName}</span>,</p>
                <p><span>{projectName}</span></p>
                <button className={styles.actionBtn} onClick={onEdit}>
                    <svg className={styles.icon} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <g clipPath="url(#clip0_5976_200431)">
